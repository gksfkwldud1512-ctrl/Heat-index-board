...
### TODO — 경광등/Tapo 기기 도착하면 반영할 로직 변경 (2026-07-07 논의 확정)
지금 코드는 실제 휴식시간 구간(T~T+10분, 예: 09:00~09:10)에 켜지는데, **휴식시간 시작을 미리 예고하는 용도로 변경 예정**:
경광등 ON 구간을 **T-30분 ~ T(휴식시간 시작 직전까지)**, 예: 09:00 판정이면 **08:30~09:00**에 켜지고 09:00이 되면 꺼짐.

9개 판정 시각 전부 서로 30분 구간이 겹치지 않아(간격 최소 2시간) 아래처럼 단순하게 구현 가능:

```js
// 지금이 어느 판정 시각(T)의 "예고 구간(T-30분~T)"에 해당하는지 찾기. 없으면 null.
function findWarningT(nowMinutes) {
  for (const T of CONFIG.FIXED_TIMES) {
    const start = T * 60 - 30;
    const end = T * 60;
    if (nowMinutes >= start && nowMinutes < end) return T;
  }
  return null;
}
```

`shouldLightBeOn()` 안에서 기존 `getActiveT`/`REVEAL_TABLE`(순환 로직) 대신 위 `findWarningT`를 사용하도록 교체하고, `T === null`이면 바로 `false` 반환. 측정창 계산(`seoulHMToUTCISO(parts, T-1, 0/30)`)과 임계값(33도) 비교 로직은 그대로 재사용.

경계 확인표 (자정 넘어가는 경우 없이 전부 같은 날짜 안에서 처리됨):
...
board.js 쪽 화면 표시 로직은 이번 변경과 무관하므로 그대로 둠.
