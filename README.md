...
폭염 대응 쉬는시간 판정 결과를 현장 화면에 크게 표시하는 프로젝트. 8개 공정에 설치된 IoT 체감온도계(Air365/케이웨더) 데이터를 기반으로 자동 판정한다.

## 지금 상태
- **실제 Air365 API 연동 완료 (2026-07-07)** — GitHub Actions가 10분마다 실제 센서 데이터를 Supabase에 저장 중.
- 계정(`jiyoung.park@johnsonelectric.com`)에 등록된 디바이스 8개 전부 활성/매핑 완료 (**2026-07-13 체감온도계 2대 추가**: `558`→1P 정형, `560`→2P 정형).
- 경광등 연동(`beacon/`) 코드도 준비됨 — 실제 스마트플러그 기기로는 아직 미검증. ⚠️ 아래 판정 규칙(측정창 T-30분~T, 반영 시점 T-10분) 변경분은 아직 `beacon/control.js`에는 반영 안 됨 — 기기 도착/검증 시 함께 맞출 것.
...
- 판정 시각 T의 측정창: **[T-30분, T)** (10분 간격 3회 데이터, 예: 09:00 판정 → 08:30/08:40/08:50)
- 측정창의 8개 공정 전체 평균 체감온도가 **33도 이상**이면 "쉬는시간 부여", 미만이면 "쉬는시간 없음"
- 화면은 각 측정창이 끝나는 시점(T-10분, 마지막 데이터 수신 시점)부터 다음 측정창이 끝날 때까지 그 판정 결과를 계속 표시 (예: 09:00 판정 결과는 08:50부터 11:00 판정이 반영되는 10:50까지 유지)
...
## 남은 절차
1. 8개 센서(1P/2P × 성형/소결/정형/후처리) 전부 매핑 완료 — collector 재실행 후 Supabase에 8개 공정 데이터가 정상 수집되는지 확인
2. GitHub Secrets는 이미 4개(`AIR365_API_KEY`, `AIR365_ID`, `SUPABASE_URL`, `SUPABASE_SERVICE_KEY`) 등록 완료, 자동 수집 중
